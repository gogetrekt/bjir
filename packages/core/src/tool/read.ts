...
import { BjirReadDedup } from "../bjir/read-dedup"
import { BjirSemanticRead } from "../bjir/semantic-read"
          ...
          execute: (input, context) => {
            return Effect.gen(function* () {
              ...
              if (
                ...
                // BJIR semantic read: large file -> symbol outline (range-read for code).
                const outline = BjirSemanticRead.outline(target, content.content)
                if (outline)
                ...
                // BJIR read-dedup: re-read of an unchanged file -> compact stub.
                const stub = BjirReadDedup.dedupe(context.sessionID, target, content.content)
                if (stub)
                  return new ReadToolFileSystem.TextPage({
                    type: "text-page",
                    content: stub,
