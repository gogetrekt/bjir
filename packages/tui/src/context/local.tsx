...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function createAgent() {
      ...
      const agentFile = path.join(paths.state, "agent.json")
      const agentState = { pending: false }
      function saveAgent() {
        if (!agentStore.ready) {
        ...
        void writeJsonAtomic(agentFile, { current: agentStore.current })
      }
      readJson<unknown>(agentFile)
        .then((x) => {
          if (x && typeof x === "object" && typeof (x as Record<string, unknown>).current === "string")
            setAgentStore("current", (x as Record<string, string>).current)
        })
        .catch(() => {})
        .finally(() => {
          setAgentStore("ready", true)
          if (agentState.pending) saveAgent()
        })
