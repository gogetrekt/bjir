  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "bjir")
        draft.active = typeof active === "string" ? active : "bjir"
        draft.ready = false
      }),
    )
    ...
    // kv.json loads asynchronously, so the synchronous read at init falls back to
    // the default. Once KV is ready, apply the persisted theme (unless config pins one).
    createEffect(() => {
      if (config.theme) return
      if (!kv.ready) return
      const saved = kv.get("theme")
      if (typeof saved === "string" && saved && hasTheme(saved)) setStore("active", saved)
    })
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "bjir")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.bjir, store.mode)
    })
