...
import { BjirCompress } from "@opencode-ai/core/bjir/compress/index"
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            // BJIR: compress tool output here — the universal live chokepoint
            // every completed tool part passes through on its way into context
            // (ContentRouter + I/O refiner; gated, never throws). Then the
            // existing safety truncation applies as a hard cap.
            const outputText = part.state.time.compacted
              ? "[Old tool result content cleared]"
              : truncateToolOutput(BjirCompress.compress(part.tool, part.state.output), options?.toolOutputMaxChars)
