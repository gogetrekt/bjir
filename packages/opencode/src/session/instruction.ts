...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const read = Effect.fnUntraced(function* (filepath: string) {
      ...
      // Inline @file references relative to THIS file's own directory, e.g. a
      // ~/.claude/CLAUDE.md containing `@RTK.md` pulls in ~/.claude/RTK.md (not
      // $cwd/RTK.md). Best-effort: one level deep, missing/duplicate refs skipped.
      ...
      for (const m of content.matchAll(/(?<![\w`])@(\.?[^\s`,.]*(?:\.[^\s`,.]+)*)/g)) {
        ...
        seen.add(name)
        const target = name.startsWith("~/") ? path.join(global.home, name.slice(2)) : path.resolve(dir, name)
        const sub = yield* fs.readFileString(target).pipe(Effect.catch(() => Effect.succeed("")))
        if (sub) extra += `\n\nReferenced file ${name} (${target}):\n${sub}`
      }
