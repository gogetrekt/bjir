...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      // BJIR: a command can declare a `model:` (e.g. inherited opencode dev
      // commands point at `opencode/*` models that aren't configured here). If
      // it's unavailable, fall back to the session model instead of hard-failing
      // with ProviderModelNotFoundError. Use provider.getModel directly so no
      // user-facing error event is published on the miss.
      const taskModel = yield* provider.getModel(preferredModel.providerID, preferredModel.modelID).pipe(
        Effect.map(() => preferredModel),
        Effect.catch(() =>
          Effect.gen(function* () {
            yield* Effect.logInfo("command model unavailable; falling back to session model", {
              command: input.command,
              requested: `${preferredModel.providerID}/${preferredModel.modelID}`,
            })
            return yield* currentModel(input.sessionID)
          }),
        ),
