  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    const options = [
      ...pipe(
        providers,
        values(),
        filter((x) => x.id !== "opencode" && x.id !== "opencode-go"),
        sortBy(
          (x) => priority[x.id] ?? 99,
          (x) => x.name ?? x.id,
        ),
        map((x) => ({
          ...
          hint: {
            ...
            "bjir-gw": "routes prompts to Claude, GPT, Gemini, or Ollama with automatic fallback — use for models you can't connect directly",
          }[x.id],
        })),
      ),
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via bjir.json options (profile, region, endpoint) or\n" +
