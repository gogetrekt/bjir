...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall BJIR and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g bjir",
      pnpm: "pnpm uninstall -g bjir",
      bun: "bun remove -g bjir",
      yarn: "yarn global remove bjir",
      brew: "brew uninstall bjir",
      choco: "choco uninstall bjir",
      scoop: "scoop uninstall bjir",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "bjir", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using BJIR!")
}
