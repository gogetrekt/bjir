...
import { BjirSemanticRead } from "@opencode-ai/core/bjir/semantic-read"
import { BjirReadDedup } from "@opencode-ai/core/bjir/read-dedup"
import { BjirProfile } from "@opencode-ai/core/bjir/profile"
import { BjirRead } from "./read.bjir"
...
>(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // BJIR read-side moat (G/G-2/G-3): on a FULL read of a text file, return a
      // symbol outline (large files; exact LSP documentSymbol, regex fallback)
      // or a dedup stub (unchanged re-reads) instead of the raw content. Range
      // reads (explicit offset/limit) bypass this and get the real lines.
      const fullRead = (params.offset ?? 1) === 1 && params.limit === undefined
      let bjirBody: string | undefined
      if (fullRead && BjirSemanticRead.enabled() && file.count > BjirSemanticRead.threshold()) {
        // G-3: prefer exact symbols from the language server.
        const symbols = yield* lsp
          .touchFile(filepath, "document")
          .pipe(
            Effect.andThen(() => lsp.documentSymbol(pathToFileURL(filepath).toString())),
            Effect.catch(() => Effect.succeed([] as unknown[])),
          )
        const lspOut = BjirRead.lspOutline(filepath, symbols, file.count)
        if (lspOut) {
          BjirProfile.record("semantic-read", Math.max(0, Number(stat.size) - lspOut.length))
          bjirBody = lspOut
        } else {
          // G-2 fallback: zero-dep regex outline over the content we already read.
          bjirBody = BjirSemanticRead.outline(filepath, file.raw.join("\n")) ?? undefined
        }
      }
      if (!bjirBody && fullRead && BjirReadDedup.enabled() && !file.more && !file.cut) {
        // G: unchanged re-read of a small/fully-read file -> compact stub.
        bjirBody = BjirReadDedup.dedupe(ctx.sessionID, filepath, file.raw.join("\n")) ?? undefined
      }
