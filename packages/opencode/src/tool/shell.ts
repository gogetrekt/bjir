...
import { BjirRtk } from "@opencode-ai/core/tool/bjir-rtk"
...
function cmd(shell: string, command: string, cwd: string, env: NodeJS.ProcessEnv) {
  // BJIR: wrap the command with rtk so its output is compressed before it
  // reaches LLM context. This is the real session shell boundary (the V2 core
  // bash tool is unused by the agent). Permission/parse already ran on the raw
  // command above; wrap() is a no-op on unsupported commands and never throws.
  command = BjirRtk.wrap(command)
