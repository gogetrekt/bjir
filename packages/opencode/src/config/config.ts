...
export interface Interface {
  ...
  readonly removeProvider: (id: string) => Effect.Effect<{ removed: boolean }>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Remove a provider block from the GLOBAL config (and clear `model` if it
    // referenced that provider). updateGlobal can only deep-MERGE, so deletion
    // needs a full rewrite — this is what `bjir revoke` uses to un-spam the
    // provider/connect lists. No-op (removed:false) if the provider isn't there.
    const removeProvider = Effect.fn("Config.removeProvider")(function* (id: string) {
      // Scan ALL global config candidates (opencode.jsonc / opencode.json /
      // config.json) — getGlobal merges them, so a provider can live in any one.
      const candidates = ["opencode.jsonc", "opencode.json", "config.json"].map((f) =>
        path.join(Global.Path.config, f),
      )
      let removed = false
      for (const file of candidates) {
        const before = yield* readConfigFile(file)
        if (!before) continue
        const parsed = ConfigParse.jsonc(before, file) as Record<string, any>
        if (!parsed?.provider || !(id in parsed.provider)) continue
        delete parsed.provider[id]
        if (Object.keys(parsed.provider).length === 0) delete parsed.provider
        if (typeof parsed.model === "string" && parsed.model.startsWith(id + "/")) delete parsed.model
        yield* fs.writeFileString(file, JSON.stringify(parsed, null, 2)).pipe(Effect.orDie)
        removed = true
      }
      if (removed) yield* invalidate()
      return { removed }
    })
